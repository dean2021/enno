...
进入基于 `tview` 的 TUI 交互界面后输入任务并按 Enter 提交。可以使用 `Esc`、`Ctrl+C`、`q` 或 `exit` 退出。主 `Enno` 窗口是单一会话流：用户输入、模型进度、工具调用、工具参数、弱化显示的工具结果和最终回答会按时间顺序追加，便于直观看到当前正在进行的动作。可以使用 `PgUp`/`PgDn`、方向键、`Home`/`End` 查看之前的输出；鼠标拖拽可用于选中并复制内容。它不会展示模型隐藏思维链。
